...
class TestRollbackManager:
    ...
    def test_git_rollback_success(self):
        ...
        with patch("subprocess.run") as mock_subprocess:
            ...
            result = manager.git_rollback(commit_hash="abc123", message="Test rollback")
    ...
    def test_file_backup_rollback_success(self):
        ...
        with patch("os.path.exists") as mock_exists, patch("shutil.copy2") as mock_copy, patch("os.remove") as mock_remove:

            # 模拟备份文件存在
            mock_exists.return_value = True

            result = manager.file_backup_rollback(file_path="/test/file.py", backup_path="/test/file.py.backup")
    ...
    def test_create_backup_before_edit(self):
        ...
        with patch("os.path.exists") as mock_exists, patch("shutil.copy2") as mock_copy, patch("os.makedirs") as mock_makedirs:
    ...
    def test_create_backup_directory_creation(self):
        ...
        with patch("os.path.exists") as mock_exists, patch("shutil.copy2"), patch("os.makedirs") as mock_makedirs:
    ...
    def test_multiple_file_rollback_partial_failure(self):
        ...
        with patch.object(manager, "file_backup_rollback") as mock_rollback:
            ...
            mock_rollback.side_effect = [
                RollbackResult(success=True, strategy=RollbackStrategy.FILE_BACKUP),
                RollbackResult(success=False, strategy=RollbackStrategy.FILE_BACKUP, error_message="Backup not found"),
            ]
    ...
    def test_smart_rollback_strategy_selection(self):
        ...
        # 测试Git仓库环境选择Git策略
        with patch.object(manager, "_is_git_repo") as mock_git_check, patch.object(manager, "git_rollback") as mock_git_rollback:

            mock_git_check.return_value = True
            mock_git_rollback.return_value = RollbackResult(success=True, strategy=RollbackStrategy.GIT)

            result = manager.smart_rollback(file_path="/test/file.py")
    ...
    def test_cleanup_old_backups(self):
        ...
        with patch("glob.glob") as mock_glob, patch("os.path.getmtime") as mock_getmtime, patch("os.remove") as mock_remove:

            # 模拟找到多个备份文件
            backup_files = ["/test/file1.py.backup", "/test/file2.py.backup", "/test/file3.py.backup"]
            mock_glob.return_value = backup_files

            # 模拟文件时间，file3.py.backup是最旧的
            mock_getmtime.side_effect = [1000, 2000, 500]

            manager.cleanup_old_backups(backup_dir="/test", max_backups=2)
