...
│   │   │   ├── _inbox/              # ⭐ 临时笔记空间（Sprint执行中）
│   │   │   │   └── YYYYMMDD-brief-desc.md  # 时间戳命名，扁平化
...
│   │   │   │   └── completion.md
│   │   │   ├── story-1.2/                        # Story 工作区
│   │   │   ├── bug-001-safe-search/              # Bug 工作区
│   │   │   ├── task-001-refactor/                # Task 工作区
│   │   │   └── spike-001-got/                    # Spike 工作区
...
```
格式: YYYYMMDD-{简短描述}.md
示例: 20251114-got-snapshot-idea.md
      20251115-pydantic-performance-discussion.md
      20251116-test-failure-investigation.md

位置: sprints/current/_inbox/
特点: 
- 时间戳前缀（YYYYMMDD），避免命名冲突
- 扁平化组织（不创建子目录）
- Sprint 结束时必须清空（提炼/移动/删除）
- 低摩擦记录：不需要思考分类，直接写
...
示例:
- story-1.2-execution-plan-integration/
- bug-001-safe-search-null-pointer/
- task-001-refactor-logging/
- spike-001-got-event-sourcing/

特点:
- 每个工作项有独立目录
- 目录内文档无需重复工作项前缀
- 简单工作项：1个文档即可（如 notes.md）
- 复杂工作项：按需拆分多个文档
```
...
2. 根据文档类型选择归档位置：
   ...
   **Sprint 结束时必做（完整流程见 workflow-checklist.md）**：
   
   a) **_inbox/ 整理**（30-45分钟）
   - 列出所有临时笔记
   - 逐个决策归宿：
     * 架构决策 → 提炼为 ADR → architecture/adrs/
     * 开发教训 → 提炼为 Lesson → knowledge/lessons-learned/
     * 技术研究 → 整理为 Research → knowledge/research/
     * 未来想法 → 移动到 product/backlog/future-ideas/
     * 个人笔记 → 移动到个人管理系统
     * 临时协调 → 删除
   - 验证 _inbox/ 清空
   ...
   **Story 工作区** → `sprints/completed/{sprint-id}/`
   - story-{编号}/ 整个目录移动
   
   **Sprint 文档** → `sprints/completed/{sprint-id}/`
   - sprint-{编号}-{描述}.md
   - 该 Sprint 的所有 Story 工作区
   - Sprint 总结报告
...
**开发工作流Checklist**
→ `docs/development/workflow-checklist.md`
...
6. **使用 _inbox/ 时**：
   - 临时想法、讨论记录、调查笔记 → _inbox/
   - 使用时间戳命名: `YYYYMMDD-brief-desc.md`
   - 不创建子目录（扁平化）
   - Sprint 执行中可以自由创建
   - Sprint 结束时提醒用户清理 _inbox/（不要自动清理）

7. **整理 _inbox/ 时**（仅在用户明确要求时）：
   - 不自动整理，等待用户决策
   - 可以建议分类方向（ADR/Lesson/Research等）
   - 不自动移动文件（用户手动执行）
   - 提炼正式文档时使用相应模板
   - 参考完整流程：`docs/development/workflow-checklist.md` Phase 5
