...
### Feature 5.5: 专家模式 - 动态深度调整 (后续开发)

**目标**: 实现类似 SequentialThinking 的动态深度调整机制

**背景**:
- 防呆模式（默认）：完整 5 步，固定深度
- 专家模式：5 步 + 动态深度，根据发现调整分析深度
...
类似 SequentialThinking 的 nextThoughtNeeded 机制
```
...
**Stories**:
- [ ] Story 5.5.1: 设计动态深度调整机制
- [ ] Story 5.5.2: 实现循环内的 "need more analysis" 判断
- [ ] Story 5.5.3: 集成 SequentialThinking 或类似工具
- [ ] Story 5.5.4: 添加专家模式触发和报告格式
